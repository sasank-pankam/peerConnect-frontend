...
const FileBox = ({fileProps}) => {
    ...
    const {
        name = '', isSender = true, size = 0, ext = '', id = 0, isAccepted = false
    } = fileProps;
    const handleClick = () => {
        new contentSenderObject(socket, {
            [consts.HEADER] : consts.ACCEPT_FILE,
            [consts.ID] : id,
            [consts.CONTENT]: 'accept',
        }).sendContent();
    };
    return (<div style={{
        height: 'fit-content',
            ...
            <div className='main-file-container bg-gray-400'>
                ...
                <div className='text-div-container'>
                    <div className='file-name-container font-semibold'>
                        <div>{name}</div>
                        {!isSender && !isAccepted && (<div onClick={handleClick}>
                            <svg viewBox="0 0 34 34" height="28" width="28" preserveAspectRatio="xMidYMid meet"
                                 className="" version="1.1" x="0px" y="0px" enableBackground="new 0 0 34 34">
...
export const getFile = (extraProps) => {
    return {
        type: consts.FILE_TYPE, ...extraProps
    }
};