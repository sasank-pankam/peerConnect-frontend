...
const ScrollToBottom = ({divRef, id}) => {
    ...
    const {counts, setCounts} = useContext(UsersContext);
    const [scroll, setScroll] = useState(divRef.current.scrollTop);
    let displayStyle
    useEffect(() => {
        displayStyle = divRef.current && divRef.current.scrollTop + divRef.current.clientHeight !== divRef.current.scrollHeight ? "block" : "none";
